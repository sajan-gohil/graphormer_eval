...
# ---------------------------------------------------------------------------
# Multi-hop masked MHA: every head masked by every hop (H x num_hop views).
# ---------------------------------------------------------------------------
class MultiHopMaskedMHA(nn.Module):
    ...
    Readout (this module, per the current design — sum/mean, not PMA):
...
class HopMaskedTransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        if use_moe_gating:
            ...
            # Every head masked by every hop (+ optional global), collapsed back
            # to hidden_dim by a sum/mean readout over the hop axis. The model
            # feeds this layer the per-hop dist_masks (B, K, N, N) directly.
            self.attn = MultiHopMaskedMHA(
                hidden_dim, num_heads, dropout, block_diag_out=block_diag_out,
                v_head_dim=v_head_dim, readout=multihop_readout,
                include_global=multihop_include_global,
            )
        else:
...
class HopMaskedTransformerModel(nn.Module):
    ...
    def forward(self, batch, dist_masks, node_masks):
        ...
        if self.use_moe_gating:
            ...
            # Multi-hop path: every head is masked by every hop. The layer's
            # MultiHopMaskedMHA consumes the per-hop dist_masks (B, K, N, N)
            # directly, so there is no per-head union mask to build.
            for layer in self.layers:
