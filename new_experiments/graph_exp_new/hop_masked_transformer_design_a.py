...
class HopMaskedMHA(nn.Module):
    ...
    def __init__(self, hidden_dim: int, num_heads: int, dropout: float = 0.0,
        ...
        # Design A: one spectral filter per attention head, operating on the
        # Laplacian of that head's hop-induced graph.  The eigendecomposition
        # is computed from the existing per-head mask, so no new graph inputs
        # or top-level plumbing are required.
        ...
        # Start as the original model; training can turn on the spectral branch.
        self.spectral_scale = nn.Parameter(torch.zeros(num_heads))

    def _hop_spectral_filter(
        self,
        v: torch.Tensor,              # (B, H, N, Dv)
        per_head_mask: torch.Tensor,  # (B, H, N, N) bool
        node_mask: torch.Tensor,      # (B, N) bool
    ) -> torch.Tensor:
        """Apply a separate Laplacian spectral filter to every hop head.

        For each graph b and head h:
            A_h -> D_h -> L_h -> (lambda_h, V_h)
            X_h -> V_h^T X_h -> g_h(lambda_h) * (...) -> V_h (...)

        The hop adjacency comes directly from the existing attention mask.
        Self loops are removed before constructing the normalized Laplacian.
        """
        B, H, N, Dv = v.shape
        valid_pairs = node_mask[:, None, :, None] & node_mask[:, None, None, :]

        # Hop-induced adjacency. Symmetrize defensively and remove self loops.
        A = (per_head_mask & valid_pairs).to(v.dtype)
        A = torch.maximum(A, A.transpose(-1, -2))
        eye = torch.eye(N, device=v.device, dtype=v.dtype).view(1, 1, N, N)
        A = A * (1.0 - eye)
        ...
        # Normalized Laplacian. Padded/isolated rows are set to zero so they do
        # not create artificial spectral content.
        L = eye - A_norm
        active = (degree > 0) & node_mask[:, None, :]
        active_pairs = active.unsqueeze(-1) & active.unsqueeze(-2)
        L = L * active_pairs.to(L.dtype)

        # Batched symmetric EVD: lambdas (B,H,N), V (B,H,N,N).
        lambdas, eigvecs = torch.linalg.eigh(L)

        # Graph Fourier transform: V^T X.
        x_hat = torch.matmul(eigvecs.transpose(-1, -2), v)

        # Independent learned response g_h(lambda) for each head.
        responses = []
        for h, filt in enumerate(self.spectral_filters):
            responses.append(filt(lambdas[:, h].unsqueeze(-1)).squeeze(-1))
        g = torch.stack(responses, dim=1)  # (B,H,N)

        # Filter in Fourier space and transform back to node space.
        x_hat = g.unsqueeze(-1) * x_hat
        out = torch.matmul(eigvecs, x_hat)
        out = out * node_mask[:, None, :, None].to(out.dtype)
        return out
    ...
    def forward(
        ...
        # Design A spectral branch: each head uses the Laplacian spectrum of
        # its own hop-induced graph, then adds the filtered value signal back
        # to that head before the existing output projection.
        spec = self._hop_spectral_filter(v, per_head_mask, node_mask)
        out = out + self.spectral_scale.view(1, H, 1, 1) * spec
