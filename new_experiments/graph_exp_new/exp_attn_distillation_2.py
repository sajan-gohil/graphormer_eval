...
def build_model(args):
    ...
    Pulls output_dim/task_level off ``args.task`` so a single ``--dataset``
    flag is enough to route between Peptides-func / Peptides-struct /
    PascalVOC-SP without further kwargs.
    """
...
def run_pretrain(args):
    ...
    for epoch in range(1, args.s1_max_epochs + 1):
        ...
        # Early stopping (direction depends on whether the metric is
        # higher-is-better, e.g. AP/F1, or lower-is-better, e.g. MAE).
...
def run_training(args):
    ...
    # Task-driven loss/metric (BCE / L1 / CE based on --dataset).
    task = args.task
    metric_label = task.metric_label
    higher_better = task.higher_is_better
    best_val_ap = -float("inf") if higher_better else float("inf")
...
def main():
    ...
    # Dataset selection (single source of truth — sets loss / output_dim /
    # encoder / metric / readout level via the LRGB registry).
    p.add_argument("--dataset", type=str, default="Peptides-func",
                   choices=["Peptides-func", "Peptides-struct", "PascalVOC-SP"],
                   help="LRGB dataset. Drives Task: loss, output_dim, encoder, "
                        "metric, and graph-vs-node head.")
    ...
    p.add_argument("--output_dim", type=int, default=10,
                   help="Ignored if --dataset is set; resolved via the LRGB registry.")
    ...
    # Resolve the LRGB Task once and stash on args so every helper that
    # already takes ``args`` automatically gets the right loss / metric /
    # predict / labels conversion. Also overwrite output_dim from the
    # registry so the CLI default never disagrees with the dataset.
