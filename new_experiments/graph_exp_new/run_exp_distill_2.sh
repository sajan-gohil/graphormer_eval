...
python exp_attn_distillation_2.py extract \
    ...
    --proxy_opt_steps 500 \
    --num_proxies 64 \
    --hidden_dim 512 --num_layers 1 --proxy_mmd_lambda 1 --batch_size 256 --num_cross_layers 1 --no-use_proxy_self_attn

# Phase 2: Train (run many times, fast — each epoch is just normal training speed)
python exp_attn_distillation_2.py train \
    --model_path checkpoints_staged/stage1_best.pt \
    --distill_weight 1 \
    --temperature 1.0  --hidden_dim 512 --num_layers 1 --num_proxies 64 --proxy_mmd_lambda 2 --batch_size 256 --num_cross_layers 1 --no-use_proxy_self_attn
