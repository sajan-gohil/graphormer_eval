...
class DynamicCrossHopMixer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,
        ...
        if self.hop_mode == "membership":
            ...
            x = torch.cat([x, tag.view(1, 1, self.H, self.Dh).expand(B, N, -1, -1)], dim=-1)
            ...
            x = torch.cat([x, tag.unsqueeze(1).expand(-1, N, -1, -1)], dim=-1)
        elif self.hop_mode == "head":
            x = torch.cat([x, self.hop_embedding.weight.view(1, 1, self.H, self.Dh).expand(B, N, -1, -1)], dim=-1)
        # else:
        #     x = torch.cat([x, self.temp_lin(x)], dim=-1)
...
class HopMaskedTransformerLayer(nn.Module):
    ...
    def forward(self, x, per_head_mask_or_dist_masks, node_mask,
        ...
        head_valid_mask: (B, H) bool — forwarded to DynamicCrossHopMixer to
            suppress degenerate heads (diameter < num_heads case).  None when
            dynamic_cross_hop is disabled or in MoE mode.
        """
