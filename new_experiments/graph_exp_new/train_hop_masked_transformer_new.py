...
from model_hop_masked_transformer_new import HopMaskedTransformerModel
...
def build_parser():
    ...
    # Per-hop identity preservation flags
    p.add_argument("--block_diag_out", action="store_true", default=False,
                   help="Use a block-diagonal attention output projection "
                        "(one (Dh,Dh) block per head) so heads are not "
                        "mixed in out_proj — preserves per-hop identity.")
    p.add_argument("--cross_hop_mlp", action="store_true", default=False,
                   help="Insert a per-layer cross-hop MLP sublayer that "
                        "mixes across the H hop slabs (per Dh feature). "
                        "Use together with --block_diag_out to cleanly "
                        "separate within-hop (attention) from across-hop "
                        "(this MLP) mixing.")
    p.add_argument("--cross_hop_expansion", type=int, default=2,
                   help="Hidden expansion factor for the cross-hop MLP.")
...
def main():
    ...
    model = HopMaskedTransformerModel(
        ...
        cross_hop_mlp=args.cross_hop_mlp,
        cross_hop_expansion=args.cross_hop_expansion,
    ).to(args.device)
    ...
    if args.checkpoint is not None:
        ...
        if plateau_scheduler is not None and "plateau_scheduler" in ckpt:
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if improved:
            ...
            if plateau_scheduler is not None:
                ckpt_dict["plateau_scheduler"] = plateau_scheduler.state_dict()
