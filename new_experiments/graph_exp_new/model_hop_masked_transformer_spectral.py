...
# ---------------------------------------------------------------------------
# Spectral residual branch + shared hop/spectral cross-scale mixer.
# ---------------------------------------------------------------------------
class SpectralBandFilter(nn.Module):
    """Low-rank spectral residual with B learnable spectral-band channels.
    ...
    def forward(self, x, eigvecs, eigvals, eigmask):
        ...
        return residual, bands


class SharedCrossScaleMixer(nn.Module):
    """Per-node attention over hop-specialized slabs and spectral-band slabs.

    Hop slabs and spectral slabs share the same Q/K/V projections and attend
    in one common scale sequence.  Only the updated hop slots are flattened
    back to hidden_dim; spectral information enters them through attention.
    """
    ...
    def __init__(
        ...
        self.scale_embedding = nn.Parameter(
            torch.randn(num_heads + num_spectral_bands, head_dim)
            * (0.02 * head_dim ** -0.5)
        )

    def forward(self, hop_x, spectral_bands):
        # hop_x: (B,N,H*Dh), spectral_bands: (B,N,S,Dh)
        B, N, _ = hop_x.shape
        hop = hop_x.view(B, N, self.H, self.Dh)
        scales = torch.cat([hop, spectral_bands], dim=2)
        scales = scales + self.scale_embedding.view(1, 1, self.H + self.S, self.Dh)

        q, k, v = self.q(scales), self.k(scales), self.v(scales)
        attn = torch.einsum("bnqd,bnkd->bnqk", q, k) * self.scale
        attn = self.drop(F.softmax(attn, dim=-1))
        out = torch.einsum("bnqk,bnkd->bnqd", attn, v)
        out = self.out(out)
        return out[:, :, :self.H].reshape(B, N, self.H * self.Dh)
...
class HopMaskedTransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        if use_spectral_branch:
            self.spectral = SpectralBandFilter(
            ...
            self.drop_spec = nn.Dropout(dropout)
            self.drop_scale = nn.Dropout(dropout)
    ...
    def forward(self, x, per_head_mask_or_dist_masks, node_mask, spectral_basis=None):
        """Forward pass.
        ...
        if self.use_spectral_branch:
            if spectral_basis is None:
                raise ValueError("spectral_basis is required when use_spectral_branch=True")
            eigvecs, eigvals, eigmask = spectral_basis
            spec_residual, spec_bands = self.spectral(normed, eigvecs, eigvals, eigmask)
            x = x + self.drop_spec(spec_residual)
            x = x + self.drop_scale(self.cross_scale(x, spec_bands))
...
class HopMaskedTransformerModel(nn.Module):
    ...
    def _build_spectral_basis(self, dist_masks, node_mask, batch=None):
        ...
        eigvals = dist_masks.new_zeros(B, K, dtype=torch.float32)
        eigmask = torch.zeros(B, K, device=dist_masks.device, dtype=torch.bool)
        ...
        with torch.no_grad():
            for b in range(B):
                ...
                if n == 0:
                    continue
                kb = min(self.spectral_k, n)
                A = dist_masks[b, 1, :n, :n].float()
                A = torch.maximum(A, A.transpose(0, 1))
                deg = A.sum(dim=-1)
                inv_sqrt = deg.clamp_min(1.0).rsqrt()
                L = torch.eye(n, device=A.device, dtype=A.dtype) - (
                    inv_sqrt[:, None] * A * inv_sqrt[None, :]
                )
                vals, vecs = torch.linalg.eigh(L)
                eigvals[b, :kb] = vals[:kb]
                eigvecs[b, :n, :kb] = vecs[:, :kb]
                eigmask[b, :kb] = True
        return eigvecs, eigvals, eigmask
    ...
    def forward(self, batch, dist_masks, node_masks):
        ...
        if self.use_moe_gating:
            ...
            for layer in self.layers:
                x, gate_weights = layer(x, mask_source, nm, spectral_basis=spectral_basis)
            ...
            for layer_idx, layer in enumerate(self.layers):
                ...
                x = layer(x, per_head_mask, nm, spectral_basis=spectral_basis)
            aux_loss = x.new_tensor(0.0)
