...
class BlockDiagLinear(nn.Module):
    ...
    independently. Equivalent to H parallel (Dh -> Dh) Linears, but stored
    as a single (H, Dh, Dh) weight tensor for batched einsum.

    Preserves per-head identity end-to-end through the output projection,
    so subsequent layers / pooling can still attribute each Dh-slice to a
    specific hop range.
    """
    ...
    def __init__(self, num_heads: int, head_dim: int):
        ...
        # Kaiming-uniform on each per-head block independently.
        for h in range(num_heads):
            nn.init.kaiming_uniform_(self.weight[h], a=math.sqrt(5))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, N, H*Dh) — incoming concat of per-head outputs.
        B, N, d = x.shape
        H, Dh = self.num_heads, self.head_dim
        x = x.view(B, N, H, Dh)
        # Per-head matmul: (B, N, H, Dh) @ (H, Dh, Dh) -> (B, N, H, Dh).
        out = torch.einsum("bnhd,hde->bnhe", x, self.weight) + self.bias
        return out.reshape(B, N, d)


# ---------------------------------------------------------------------------
# Cross-hop MLP: mixes across the H (hop) axis only, keeps Dh axis pointwise.
# ---------------------------------------------------------------------------
class CrossHopMLP(nn.Module):
    """Per-node MLP that mixes information across the H hop-tagged slabs.

    Operates on x viewed as (B, N, H, Dh) and applies a 2-layer MLP along
    the H axis, sharing weights across the Dh feature dim. This is the
    explicit cross-hop interaction that stacked global attention / FFN
    only learn implicitly (and inefficiently) because the standard FFN
    mixes the full d=H*Dh axis with no notion of "this dim belongs to
    hop k".
    """

    def __init__(self, num_heads: int, head_dim: int, expansion: int = 2,
                 dropout: float = 0.0):
        super().__init__()
        self.num_heads = num_heads
        self.head_dim = head_dim
        H = num_heads
        self.fc1 = nn.Linear(H, H * expansion)
        self.fc2 = nn.Linear(H * expansion, H)
        self.act = nn.GELU()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B, N, d = x.shape
        H, Dh = self.num_heads, self.head_dim
        x = x.view(B, N, H, Dh).transpose(-1, -2)   # (B, N, Dh, H)
        x = self.fc2(self.drop(self.act(self.fc1(x))))  # (B, N, Dh, H)
        x = x.transpose(-1, -2).reshape(B, N, d)    # (B, N, H*Dh)
        return x
...
class HopMaskedMHA(nn.Module):
    ...
    Args:
        block_diag_out: if True, use a block-diagonal output projection
            ...
            out_proj. This preserves per-head/per-hop identity through
            the layer boundary at the cost of giving up cross-head
            mixing inside the attention sublayer (the cross_hop_mlp /
            FFN are responsible for mixing instead).
    """
...
class HopMaskedTransformerLayer(nn.Module):
    ...
    With ``cross_hop_mlp=True`` an extra sublayer is inserted between
    attention and FFN:
        x = x + attn(norm1(x))
        x = x + cross_hop_mlp(norm_ch(x))   # mixes across H hop slabs
        x = x + ffn(norm2(x))               # mixes across all H*Dh dims

    Pairing ``block_diag_out=True`` (no cross-head mixing inside attention)
    with ``cross_hop_mlp=True`` gives a clean separation: attention is the
    "within-hop" mixer, the cross-hop MLP is the "across-hop" mixer, and
    the FFN remains the pointwise channel mixer.
    """
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        block_diag_out: bool = False,
        cross_hop_mlp: bool = False,
        cross_hop_expansion: int = 2,
    ):
        super().__init__()
        ...
        self.use_cross_hop = cross_hop_mlp
        if cross_hop_mlp:
            head_dim = hidden_dim // num_heads
            self.norm_ch = nn.LayerNorm(hidden_dim)
            self.cross_hop = CrossHopMLP(
                num_heads=num_heads,
                head_dim=head_dim,
                expansion=cross_hop_expansion,
                dropout=dropout,
            )
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList([
            HopMaskedTransformerLayer(
                ...
                cross_hop_mlp=cross_hop_mlp,
                cross_hop_expansion=cross_hop_expansion,
            )
