...
def pcgrad_project(grads_main, grads_aux, params):
    """
    Apply PCGrad projection: remove conflicting component of auxiliary gradient.
    
    Args:
        grads_main: Gradients from primary (task) loss
        grads_aux: Gradients from auxiliary (struct) loss
        params: Model parameters
    
    Returns:
        Projected auxiliary gradients
    """
    g1 = flatten_grads(grads_main)
    g2 = flatten_grads(grads_aux)
    
    # Check for gradient conflict
    dot = torch.dot(g1, g2)
    
    if dot < 0:
        # Project auxiliary gradient to remove conflict
        proj = dot / (torch.dot(g1, g1) + 1e-8)
        grads_aux_proj = [
            g_aux - proj * g_main
            for g_aux, g_main in zip(grads_aux, grads_main)
        ]
        return grads_aux_proj
    else:
        return grads_aux
...
def train_epoch(model, loader, optimizer, device, attn_loss_weight=0.1):
    ...
    for data in loader:
        ...
        # Task loss (binary cross-entropy) - PRIMARY
        task_loss = F.binary_cross_entropy_with_logits(out, data.y)
        
        # Attention improvement loss - AUXILIARY (used in PCGrad)
        ...
        # PCGrad: Compute gradients separately and project
        if attn_loss_weight > 0:
            # Compute gradients for primary loss (task)
            grads_main = torch.autograd.grad(
                task_loss, params, retain_graph=True, create_graph=False
            )
            
            # Compute gradients for auxiliary loss (attention improvement)
            grads_aux = torch.autograd.grad(
                attn_loss, params, retain_graph=False, create_graph=False, allow_unused=True
            )
            ...
            # Combine gradients: g = g_main + lambda * g_aux_projected
            final_grads = [
                g_main + attn_loss_weight * g_aux
                for g_main, g_aux in zip(grads_main, grads_aux_proj)
            ]
            ...
            # No attn loss, just backprop task loss
            task_loss.backward()
        
        optimizer.step()
        
        # Compute effective loss for logging
        loss = task_loss.item() + attn_loss_weight * attn_loss.item()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--attn_loss_weight", type=float, default=0,
                        help="Weight for attention improvement loss (auxiliary loss in PCGrad)")
