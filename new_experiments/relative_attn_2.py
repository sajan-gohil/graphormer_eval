...
class GraphMultiHeadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout, use_diffusion=False, diffusion_steps=3):
        super().__init__()
    ...
    def forward(self, x, edge_index, return_qkv=False):
        ...
        # relative-feature attention scores
        q = Q.unsqueeze(2)           # [H, N, 1, D]
        k = K.unsqueeze(1)           # [H, 1, N, D]
        dist2 = (q - k).pow(2).sum(dim=-1)
        scores = -dist2 / (self.sigma.abs() + 1e-6)

        attn = F.softmax(scores, dim=-1)
        attn = self.dropout(attn)

        # diffusion = repeated application of same kernel
        if self.use_diffusion:
            for _ in range(self.diffusion_steps - 1):
                attn = torch.matmul(attn, attn)

        out = torch.matmul(attn, V)          # [H, N, D]
...
class GraphTransformerLayer(nn.Module):
    def __init__(self, dim, heads, dropout, use_diffusion=False):
        super().__init__()
        self.attn = GraphMultiHeadAttention(
            dim, heads, dropout,
            use_diffusion=use_diffusion  # default, overridden at top level
        )
...
class GraphTransformer(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, layers, heads, dropout, 
        ...
        self.layers = nn.ModuleList([
            GraphTransformerLayer(hidden_dim, heads, dropout, use_diffusion)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_diffusion", action="store_true",
                    help="Enable diffusion over relative-feature attention")
