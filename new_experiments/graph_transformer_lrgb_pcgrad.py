...
def train_epoch(model, loader, optimizer, device, attn_loss_weight=0.1, struct_loss_weight=0.1):
    ...
    for data in loader:
        ...
        # Attention improvement loss (added to task loss)
        ...
        # Combined primary loss (task + attention)
        primary_loss = task_loss + attn_loss_weight * attn_loss
        
        # Structure reconstruction loss - AUXILIARY
        ...
        if struct_loss_weight > 0 and first_layer_emb is not None:
            # Compute gradients for primary loss (task + attn)
            grads_main = torch.autograd.grad(
                primary_loss, params, retain_graph=True, create_graph=False
            )
            ...
            final_grads = [
                g_main + struct_loss_weight * g_aux
            ...
            # No struct loss, just backprop primary loss
            primary_loss.backward()
        
        optimizer.step()
        
        # Compute effective loss for logging
        loss = primary_loss.item() + struct_loss_weight * struct_loss.item()
