...
class DDPMScheduler:
    ...
    def ddim_sample(self, denoiser, x_enc, x_mask,
        ...
        """
        Uses all T steps (num_steps == T) for a clean, consistent reverse pass.
        Since T=50 this is fast and there is no train/infer mismatch.
        """
        B      = x_enc.shape[0]
        device = x_enc.device
        ...
        for i, t_val in enumerate(ts):
            ...
            eps_pred = denoiser(x_t, t_batch, x_enc, x_mask)
            t_cur    = t_val.item() + 1
            t_prev   = int(ts[i + 1].item()) + 1 if i + 1 < len(ts) else 0
            x0_pred  = (x_t - self.sqrt_one_minus[t_cur] * eps_pred) / self.sqrt_ab[t_cur]
            x0_pred  = x0_pred.clamp(-10, 10)
            x_t      = self.sqrt_ab[t_prev] * x0_pred + self.sqrt_one_minus[t_prev] * eps_pred
...
def phase3_train_diffusion(proxy_pairs, epochs=500, lr=1e-4,
    ...
    for epoch in range(epochs):
        ...
        for x_enc, x_mask, b0 in tqdm(loader, desc=f"Diffusion ep {epoch+1}", leave=False):
            ...
            b_noisy, eps = sched.q_sample(b0, t)
            eps_pred     = denoiser(b_noisy, t, x_enc, x_mask)
...
def phase4_train_with_proxies(denoiser, sched, train_loader, val_loader, test_loader,
                               ...
                               warmup_epochs=50):
    print("\n" + "=" * 60)
    print("PHASE 4: GT Backbone + Diffusion Proxy Nodes (finetuning from P1)")
    print("=" * 60)
    ...
    # Initialize from Phase 1 pretrained weights for consistent conditioning
    ...
    # Freeze encoder (Atom+Bond) — only finetune transformer layers + head
    _freeze(backbone.encoder)
    print("  Encoder (Atom+Bond) frozen — finetuning transformer layers + head")
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_loader,
                          desc=f"P4 {epoch+1:03d} {'[proxy]' if use_proxies else '[warmup]'}",
                          leave=False):
            batch = batch.to(DEVICE)
            if use_proxies:
                backbone.eval()
...
if __name__ == "__main__":
    ...
    if os.path.exists(PROXY_PATH):
        ...
        proxy_pairs, sample_improved = phase2_optimize_proxies(
            ...
            num_proxy=NUM_PROXY, max_steps=1000, proxy_lr=1e-3, num_repeats=5,
        )
    ...
    # ── Phase 4 (full self-attention with proxies) ───────────────
    ...
    if os.path.exists(P4_CKPT):
        ...
        p4_backbone, p4_hist = phase4_train_with_proxies(
